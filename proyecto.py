...
def parallel_multiprocessing_dotplot(sequence1, sequence2, threads=mp.cpu_count()):
    args = [(i, sequence1, sequence2) for i in range(len(sequence1))]
    with mp.Pool(processes=threads) as pool:
        dotplot = list(pool.map(worker_multiprocessing, args))
    return dotplot


def save_results_to_file(results, file_name="results/results.txt"):
...
def parallel_mpi_dotplot(sequence_1, sequence_2):
    ...
    chunks = np.array_split(np.arange(len(sequence_1)), size)

    dotplot = np.zeros((len(chunks[rank]), len(sequence_2)), dtype=np.float16)
    ...
    gathered_dotplots = comm.gather(dotplot, root=0)

    if rank == 0:
        merged_data = np.vstack(gathered_dotplots)
        return merged_data
...
def main():
    ...
    if rank == 0:
        ...
        save_results_to_file([F"Tiempo de carga de los archivos: {chargeFilesFinish - chargeFilesStart}"],file_name="results/results_charge_files.txt")
    ...
    if args.sequential:
        ...
        save_results_to_file(results_print,file_name="results/results_sequential.txt")
    ...
    if args.multiprocessing:
        ...
        save_results_to_file(results_print,file_name="results/results_multiprocessing.txt")
    ...
    if args.mpi:
        ...
        save_results_to_file(results_print_mpi,file_name="results/results_mpi.txt")
        draw_graphic_mpi(
